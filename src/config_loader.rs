...
pub fn config_thread<C: ConfigFile<L> + Debug, L: Live>(
    ...
    let mut settings_file = File::open(&path.config)?;
    ...
    watcher.watch(&path.config, RecursiveMode::NonRecursive)?;
    ...
    thread::Builder::new()
        ...
        .spawn(move || -> () {
            loop {
                ...
                let data = match fs::read(&path.config) {
                    Ok(d) => d,
                    Err(_) => continue,
                };
