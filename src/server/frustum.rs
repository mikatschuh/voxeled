...
impl Frustum {
    pub fn chunk_ids(self) -> IntoIter<ChunkID> {
        ...
        let now = Instant::now();
        let chunks = self.flood_fill().iter().map(|c| c.pos).collect::<Vec<_>>();
        println!("frustum_flood_fill: {}", now.elapsed().as_micros());
            ...
            if in_frustum(chunk) {
                ...
                for neighbor in chunk_neighbors(chunk) {
                    if already_queued.insert(neighbor) {
                        let lod = lod_level_at(cam_pos, (neighbor.total_pos() & !1).as_vec3());
