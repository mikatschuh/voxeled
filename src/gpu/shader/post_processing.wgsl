@vertex
fn full_screen_quat(@builtin(vertex_index) vertex_index: u32) -> @builtin(position) vec4<f32> {
...
@fragment
fn apply_effects(in: UVCoord) -> @location(0) vec4<f32> {
    let color = textureSample(prev_img, prev_img_s, in.pos.xy);
